...
if (!dir.exists("04_CYT/")) {dir.create("04_CYT/")}
setwd("04_CYT/")
...
write.table(
  train_data, 
  file = "train_data_filtered.txt",  # 文件名
  sep = "\t",     # 制表符分隔（标准txt格式）
  quote = FALSE,  # 去掉多余引号
...
# 2. 提取GZMA与PRF1
gzma <- train_data["GZMA", ]
prf1 <- train_data["PRF1", ]


# 3. 计算CYT评分
cyt_score <- sqrt(gzma * prf1)
rownames(cyt_score) <- "CYT"  
rownames(cyt_score)


res.cibersort <- as.data.frame(t(cyt_score))
res.cibersort$sample <- rownames(res.cibersort)
